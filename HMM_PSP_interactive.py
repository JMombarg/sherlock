...
with open(f'gyre_per_l1m{m}_ext.pkl', 'rb') as f:
    x = pickle.load(f)
...
def read_frequency_list(KIC, combinations_included = True):
    ...
    '''
    strat =  pd.read_csv('best_strategy.txt', names = ['KIC', 'strategy'], sep=' ', dtype = str)
    KIC_list = np.array(strat['KIC'])
    strategy = strat['strategy'][np.where(KIC_list == KIC)[0][0]]
    df = pd.read_csv(f'forJoey/amplitudes_frequencies_phases_KIC0{KIC}_strategy_{strategy}.asc', sep = '\t', header = 9, names = ['freq', 'sigma_freq', 'ampl', 'sigma_ampl', 'phase', 'sigma_phase', 'nr', 'nonlin_id'])
